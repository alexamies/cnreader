...
func ReadDict(wsFilenames []string, validator Validator,
		dictConfig DictionaryConfig) ([]HeadwordDef, error) {
	wdict = make(map[string][]*WordSenseEntry)
	avoidSub := dictConfig.AvoidSubDomains
...
func WriteHeadwords(dictConfig DictionaryConfig) {

	hwArray := GetHeadwords()

	// Prepare head words file for writing
	outfile := dictConfig.DictionaryDir + "/" + HEADWORD_FILE
